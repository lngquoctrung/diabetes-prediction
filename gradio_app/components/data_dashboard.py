...
def create_diabetes_distribution_pie():
    ...
    fig.update_layout(title="Diabetes Distribution", height=400, showlegend=True)
    ...
    with gr.Column():
            ...
            with gr.Column(scale=1):
                gr.HTML(f"""
                    <div style='background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px; border-radius: 10px; color: white;'>
                        <div style='font-size: 0.9em;'>Total Samples</div>
                        <div style='font-size: 2em; font-weight: bold;'>{stats['total_samples']:,}</div>
                    </div>
                """)
            ...
            with gr.Column(scale=1):
                gr.HTML(f"""
                    <div style='background: linear-gradient(135deg, #f093fb 0%, #f5576c 100%); padding: 20px; border-radius: 10px; color: white;'>
                        <div style='font-size: 0.9em;'>Features</div>
                        <div style='font-size: 2em; font-weight: bold;'>{stats['total_features']}</div>
                    </div>
                """)
            ...
            with gr.Column(scale=1):
                gr.HTML(f"""
                    <div style='background: linear-gradient(135deg, #4facfe 0%, #00f2fe 100%); padding: 20px; border-radius: 10px; color: white;'>
                        <div style='font-size: 0.9em;'>Diabetes Rate</div>
                        <div style='font-size: 2em; font-weight: bold;'>{stats['diabetes_rate']:.1f}%</div>
                    </div>
                """)
        ...
        with gr.Tabs():
            ...
            with gr.Tab("Data Viewer"):
                ...
                with gr.Row():
                    page_size_dropdown = gr.Dropdown(choices=[50, 100], value=50, label="Rows per page")
                ...
                first_btn.click(fn=go_first, inputs=[page_size_state], outputs=[data_table, page_info, current_page_state])
                prev_btn.click(fn=go_prev, inputs=[current_page_state, page_size_state], outputs=[data_table, page_info, current_page_state])
                next_btn.click(fn=go_next, inputs=[current_page_state, page_size_state], outputs=[data_table, page_info, current_page_state])
                last_btn.click(fn=go_last, inputs=[page_size_state], outputs=[data_table, page_info, current_page_state])
                page_size_dropdown.change(fn=change_page_size, inputs=[page_size_dropdown], outputs=[data_table, page_info, current_page_state])
                page_size_dropdown.change(fn=lambda x: x, inputs=[page_size_dropdown], outputs=[page_size_state])
